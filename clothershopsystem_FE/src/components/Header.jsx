...
<div className="brand">
        <Link to="/">
          <img src={logoImg} alt="Nike Logo" className="logo-image" height={100} width={100}/>
        </Link>
      </div>
      <nav className="site-nav" aria-label="Primary navigation">
        {categories.map((cat) => (
          ...
          >
            <Link to={`/category/${cat.categoryId}`} className="nav-link">
              {cat.name}
            </Link>
          </div>
        ))}
              ...
              <ul>
                {level2.children?.map((level3) => (
                  <li key={level3.categoryId}>
                    <Link to={`/category/${level3.categoryId}`}>
                      {level3.name}
                    </Link>
                  </li>
                ))}
